...
export default function Dashboard() {
	...
	React.useEffect(() => {
		...
		async function fetchTeam() {
			...
			setMyTeam(formattedData);
			setSelectedTeam(formattedData);
		}

		async function fetchTeams() {
			const response = await teamService.getTeams();
			const formattedData = response.data.map(({ username, players }) => {
				...
				return { username, players: formattedPlayers };
			});
	...
	return (
		<ModalProvider>
					...
					<div className="flex flex-col h-[calc(100vh-200px)] overflow-y-auto pr-4">
						<div className="flex justify-between pb-4">
							<h1 className="w-[318px] ml-10 text-left mt-2">
								{selectedTeam.username || username}'s Team
							</h1>
						</div>
						<div className="flex flex-row justify-center flex-wrap gap-[1rem] pt-4">
							{(selectedTeam.players || selectedTeam).map((teamPlayer) => {
			...
			<main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8 h-[93vh] w-full bg-background">
				<div className="hidden sm:flex flex-wrap justify-between gap-4">
				...
				<div
					className={`grid flex-1 gap-4 md:gap-8 ${user ? "lg:grid-cols-2" : ""} ${user ? "xl:grid-cols-3" : ""}`}
				>
							...
							<Table>
								<TableHeader>
									<TableRow>
										...
										<TableHead className="text-center hidden xl:flex justify-center items-center">
											Players
										</TableHead>
								...
								<TableBody>
									{teams
										...
										.map((team) => {
											return (
												<TableRow key={team.username} onClick={() => setSelectedTeam(team)}>
													<TableCell>
														<ModalTrigger>
															...
															<TooltipProvider>
																<div className="flex flex-row items-center gap-4">
																	{team.players
																		.sort((a, b) => {
																			const valueA =
																				playerFantasyPoints[a.id] || 0;
																			const valueB =
																				playerFantasyPoints[b.id] || 0;
																		...
																		.map((player) => (
																			<Tooltip key={player.id}>
																				<TooltipTrigger asChild>
																					<Avatar className="h-9 w-11 sm:flex">
																						<AvatarImage
													...
													<TableCell className="text-right">
														<ModalTrigger>
															{team.players
																.reduce(
																	(acc, cur) =>
																		acc + (playerFantasyPoints[cur.id] || 0),
																	0,
																)
																.toFixed(2)}
														</ModalTrigger>
