...
def scrape_stories():
    ...
    if response.status_code == 200:
        ...
        articles = soup.findAll(
            'article', {'class': re.compile(r'ArticleTile_tile')})
        stories = []

        for article in articles:
            id = f'{uuid.uuid4()}'
            link = 'https://www.nba.com' + article.find('a')['href']
            img_tag = article.find('img')
            if img_tag and 'src' in img_tag.attrs:
                src = img_tag['src']
                title = article.find('h3').text if article.find(
                    'h3') else 'No title'
                description = article.find('p').text if article.find(
                    'p') else 'No description'
                filename = f"public/news_images/{id}.jpg"

                download_image(src, filename)

                story = {"id": id, "img": filename, "title": title,
